...
export class FirestoreService {
  ...
  upsert<T>(ref: CollectionPredicate<T>, data): Promise<firebase.firestore.DocumentReference> {
    const timestamp = this.timestamp;
    return this.col(ref).add({
      ...data,
      updatedAt: timestamp,
  ...
  // returns a documents reference mapped to AngularFirestoreDocument
  ...
  atomic() {
    ...
    const profileDoc = firebase.firestore().doc('profiles/example_john');
    const userDoc = firebase.firestore().doc('users/uid');

    const currentTime = this.timestamp;

    batch.update(profileDoc, { timestamp: currentTime });
 ...
 * from http://github.com/AngularFirebase/80-delete-firestore-collections/blob/master/src/app/firestore.service.ts
 */
